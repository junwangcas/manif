 ...
 *
 *      u = (vx*dt, 0, w*dt) // x/y/z符合右手坐标系，加速度直接在x上，合理。
 *
 ...
 *
 *  This noise accounts for possible lateral slippage // 这个模型中，还考虑了横向打滑，就是通过这个covariance。
 *  through non-zero values of sigma_s.
 *
 *  At the arrival of a control u, a new robot pose is created at; // 这个u是local系下的tangent space.
 *
 *      X_j = X_i * Exp(u) = X_i + u. // TODO 这个为什么要直接相加呢？这个地方的表述是不是有问题
 *
 ...
 *
 *      y = (yx, yy)           // lmk coordinates in robot frame // 类似camera frame
 *
 *  Their noise n is zero mean Gaussian, and is specified
 *  with a covariances matrix R.
 *  We notice the rigid motion action y_ik = h(X_i,b_k) = X_i^-1 * b_k // 这个模型非常简单，就是把global 系转换到camera frame系
 *  (see appendix D).
 *
 *
 ...
 *      - absolute pose measurement from X0 to * (the origin of coordinates) // 在这个里面，是如何fix住x0的？ TODO
 *
 ...
 *
 *    - for the prior factor: // 这个地方倒是没说明白是怎么加入的。
 *        p_0     = X_0
 *
 *    - for the motion factors: // 这个地方就是采用标准的方法。
 *        d_ij    = X_j (-) X_i = log(X_i.inv * X_j)  // motion expectation equation
 *
 *    - for the measurement factors: // 这个地方也是标准方法
